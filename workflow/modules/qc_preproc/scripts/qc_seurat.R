...
out_root       <- "/datos/sensence/emilio/scall/results/qc_preproc/preintegrated"
...
for (i in seq_len(nrow(df))) {
  ...
  rds_path <- file.path(proj_out, paste0(sample, "_20_qc.rds"))
  saveRDS(obj, file = rds_path)
  
  if (isTRUE(convert_h5ad)) {
    # Make sure RNA is default and that feature names are unique (Scanpy requirement)
    DefaultAssay(obj) <- "RNA"
    rownames(obj) <- make.unique(rownames(obj))
    #Convert to summarizedexperiment
    
    sce <- as.SingleCellExperiment(obj)  # Seurat v5 supported
    
    # Tell zellkonverter what should become X; your NormalizeData() creates log-normalized values
    out_h5ad <- file.path(proj_out, paste0(sample, "_20_qc.h5ad"))
    zellkonverter::writeH5AD(
      sce,
      file      = out_h5ad,
      X_name    = "logcounts",   # use normalized data as X (common for Scanpy)
      compression = "gzip"
    )
  }
}
